...
namespace UnitySpine.Editor.Spine.Editor.JSONExtention
{
    public static class JsonExtention
    {
        public static T TryGet<T>(this JArray array, string name, JsonSerializer serializer, T defaultValue = default (T))
        {
            ...
            if (token == null || token.Type == JTokenType.Null || token.Type == JTokenType.None || token.Type == JTokenType.Undefined)
                return defaultValue;
        ...
        public static T TryGet<T>(this JObject obj, string name, JsonSerializer serializer, T defaultValue = default(T))
        {
        ...
        public static Color TryGetColor(this JObject obj, string name, JsonSerializer serializer, string defaultValue)
        {
            var token = obj.GetValue(name);
            var colorString = defaultValue;
            if (token == null || token.Type == JTokenType.Null || token.Type == JTokenType.None || token.Type == JTokenType.Undefined)
        ...
        public static TEnum TryGetEnum<TEnum>(this JObject obj, string name, JsonSerializer serializer,
            TEnum defaultValue) where TEnum : struct
        {
            var token = obj.GetValue(name);
            if (token == null || token.Type == JTokenType.Null || token.Type == JTokenType.None || token.Type == JTokenType.Undefined)
